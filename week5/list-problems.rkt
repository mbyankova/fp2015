...
(define (list-ref2 items n)
  (define (list-iter items current)
    (cond
      [(empty? items) "Invalid index"]
      [(= current n) (first items)]
      [else (list-iter (rest items) (+ 1 current))]))
  (list-iter items 0))
...
(define (reverse2 items)
  (define (iter result items)
    (cond
      [(empty? items) result]
      [else (iter (cons (first items) result) (rest items))]))
  (iter (list) items))
...
(define (take2 n items)
  (define (iter i result items)
   (cond
      [(> i n) (reverse2 result)]
      [else (iter (+ i 1) (cons (first items) result) (rest items))]))
  (if (> n (length items))
      items
      (iter 0 (list) items)))
...
(define (number->list n)
  (define (iter l n)
    (cond
      [(zero? n) l]
      [else (iter (cons (remainder n 10) l) (quotient n 10))]))
  (iter (list) n))
...
(define (list->number ns)
  (define (iter ns n)
    (cond
      [(empty? ns) n]
      [else (iter (rest ns) (+ (first ns) (* 10 n)))]))
  (iter ns 0))